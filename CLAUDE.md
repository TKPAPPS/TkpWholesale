...
and the caret could not be positioned. The customer reported it as "the typing area is behind
the number". With a text input the hook's `onFocus` puts the caret at the END of the value, so
backspace deletes immediately and digits can be appended (type "0" to turn 1 into 10).
**It deliberately does not select-all** — that was tried first and rejected, because
highlighting forces a full retype to change 1 into 10 and one stray keystroke wipes the
quantity. `inputMode="numeric"` keeps the phone number pad, and dropping
...
## PDFs: invoice from Odoo, order generated here
...
The order PDF is therefore **generated by the portal** with `pdf-lib` (`src/lib/order-pdf.ts`),
not fetched from Odoo. This is also the file that will be attached to the delivery email, so it
...
- **English only, deliberately.** The standard PDF fonts are Latin-1; Hebrew would need an
  ...
  than allowed to throw and fail the whole download (a Thai character in a product name would).
- **Columns are right-aligned by their right edge.** Keep a gap between Delivered and Amount;
  at the original spacing they collided and read as "6 THB 3,510.00".
- **Paginates with a repeated table header.** Verified on a real 49-line order (3 pages).
