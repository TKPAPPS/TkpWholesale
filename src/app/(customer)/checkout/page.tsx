        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-4">
            ...
            <div className="space-y-2">
              {review.delivery_addresses.map((addr) => (
                <label key={addr.id} className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${selectedAddress === addr.id ? 'border-brand-500 bg-brand-50' : 'border-gray-200 hover:border-gray-300'}`}>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-4">
            ...
            <textarea
              ...
              rows={3}
              maxLength={500}
              className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 resize-none"
            />
          </div>
          ...
          <div className="text-center">
            <Link href="/cart" className="text-sm text-brand-600 hover:underline">{t(lang, 'common.back')} to cart</Link>
          </div>
        </div>
      )}
    </div>
